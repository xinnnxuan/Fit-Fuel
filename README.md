...
The application is configured for deployment on Render. The server serves both the API and static files (HTML, CSS, JS, images) from a single service.
...
2. **Create a MongoDB Atlas Database** (if you don't have one)
   - Go to [MongoDB Atlas](https://www.mongodb.com/cloud/atlas)
   - Create a free cluster
   - Get your connection string (looks like: `mongodb+srv://username:password@cluster.mongodb.net/fitfuel`)
...
3. **Deploy on Render**
   ...
   - Or manually configure:
     - **Name**: `fitfuel-app` (or your preferred name)
     - **Environment**: `Node`
...
4. **Set Environment Variables in Render Dashboard**
   ...
   - Add the following variables:
     ...
     - `FRONTEND_ORIGIN`: (Optional) Your Render app URL (e.g., `https://fitfuel-app.onrender.com`)
...
- **Static Files**: The server now serves all static files (HTML, CSS, JS, images) from the root directory
- **API Routes**: All API endpoints are prefixed with `/api`
- **SPA Routing**: The app handles client-side routing automatically
- **CORS**: Configured to work with same-origin requests in production
- **Database**: Make sure your MongoDB Atlas cluster allows connections from anywhere (0.0.0.0/0) or add Render's IP addresses
...
- **Database connection fails**: Verify your `MONGO_URI` is correct and MongoDB Atlas allows connections
- **Static files not loading**: Ensure the server is serving from the correct directory (parent of `server/`)
